...
public class AjoutFactureAvoirClientPF extends JLayeredPane implements Constantes{
	...
	private FactureAvoirClientPF factureAvoirpf=new FactureAvoirClientPF();
...
private StockPFDAO stockpfDAO;
	 ...
	 */
	public AjoutFactureAvoirClientPF() {
		setOpaque(true);
       ...
       tableArticle.setRowHeight(20);
		...
		buttonvalider.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				FactureAvoirClientPF factureAvoirPF=factureAvoirClientpfdao.findByNumBLByDepot(txtnumfacture.getText(),depot);
			
		if(factureAvoirPF==null)
		{
			
		ClientPF clientpf=mapClientPF.get(comboClient.getSelectedItem());
		String codeTransfert=Utils.genererCodeTransfer(depot.getCode(),ETAT_TRANSFER_STOCK_AVOIR_CLIENT);
		
		factureAvoirpf.setNumBL (txtnumfacture.getText());
		factureAvoirpf.setClientPF(clientpf);
	    ...
	    factureAvoirClientpfdao.add(factureAvoirpf);
	    for(int i=0;i<listDetailFactureAvoirClientpf.size();i++)
		 {
			 detailFactureAvoirClientpfdao.add(listDetailFactureAvoirClientpf.get(i));
		 }
	    ...
	    detailcompteclient.setDesignation("Montant sur Facture Avoir N "+factureAvoirpf.getNumFacture());
	    detailcompteclient.setFactureAvoirClientpf(factureAvoirpf);
	   ...
	   /* Sequenceur  sequenceur=sequenceurDAO.findByCodeLibelle(date, Constantes.CODE_FACTURE_VENTE);
		...
		transferStock.setStatut(ETAT_TRANSFER_STOCK_AVOIR_CLIENT);
	    ...
	    factureAvoirpf=new FactureAvoirClientPF();
		...
		}else
		{
			 ...
			 JOptionPane.showMessageDialog(null, "N° de BL existe déja ","Erreur",JOptionPane.ERROR_MESSAGE);
			 return;
		...
		lblNumBl.setBounds(10, 13, 89, 24);
		layeredPane_1.add(lblNumBl);
		...
		label_3.setFont(new Font("Tahoma", Font.PLAIN, 12));
		  ...
		  combodepot.setBounds(726, 13, 341, 24);
		  ...
		  JLabel lblFournisseur = new JLabel("Client :");
		   	...
		   	public void itemStateChanged(ItemEvent e) {
   	 		 ...
   	 		 if(e.getStateChange() == ItemEvent.SELECTED)
   	 		 {
   	 			int i=0;
   	 				...
   	 				if(!comboClient.getSelectedItem().equals("") && comboClient.getSelectedIndex()!=-1 )
    			{
    				ClientPF clientpf=mapClientPF.get(comboClient.getSelectedItem());
    				if(clientpf!=null)
    				{
    					txtcodefournisseur.setText(clientpf.getCode());
		  ...
		  comboClient.setBounds(726, 49, 341, 24);
		  ...
		  txtcodefournisseur.addKeyListener(new KeyAdapter() {
		  	...
		  	public void keyPressed(KeyEvent arg0) {
		  		...
		  		{
		  			ClientPF clientpf=mapCodeClientPF.get(txtcodefournisseur.getText());
		  			if(clientpf!=null)
		  			{
		  				comboClient.setSelectedItem(clientpf.getNom());
		  				
		  			}else
		  			{
		  				JOptionPane.showMessageDialog(null, "Client PF introuvable !!!!!","Erreur",JOptionPane.ERROR_MESSAGE);
		  				return;
		  			}
							...
							if(article.getCentreCout2().equals(Constantes.ARTICLE_PACKET))
							{
				        	 ...
				        	 StockPF stockpf=stockpfDAO.findStockByMagasinPFBySousFamille(article.getId(), magasin.getId(),sousfamille.getId());
							        ...
							        detailFacture.setQuantite(QuantiteTotal);
							           ...
							           detailFacture.setFactureAvoirClientPf(factureAvoirpf);
									        ...
									        while(i<listDetailFactureAvoirClientpf.size())
									        {
											  ...
											  /*        montanttotal=  montanttotal.add(detailFactureAvoirpf.getMontantTTC().divide(detailFactureAvoirpf.getArticle().getConditionnement(),RoundingMode.HALF_UP));
									        	...
									        	}else
									        	{
									        	  ...
									        	  DetailFactureAvoirClientPF detailFactureAvoir=listDetailFactureAvoirClientpf.get(i);
	...
	void afficher_tableDetailFactureAchat(List<DetailFactureAvoirClientPF> listDetailFacture)
	{
		...
		{	
			boolean existe=false;
		DetailFactureAvoirClientPF detailfactureAchat=listDetailFacture.get(i);
