...
public class ConsulterEtatDeMargeStock extends JLayeredPane implements Constantes{
...
private DetailTransferProduitFiniDAO detailTransferProduitFiniDAO;
	 ...
	 */
	public ConsulterEtatDeMargeStock() {
		setOpaque(true);
       ...
       tableEtatStock.setModel(new DefaultTableModel(
       	...
       	new String[] {
       			"MATIERE PREMIERE","Article","Famille", "Sous Famille", "Quantite Initial", "Prix Initial","Montant Initial", "Quantite Achat", "Prix Achat","Montant Achat","Quantite Production Entrer", "Prix Production Entrer","Montant Production Entrer","Quantite Production Sortie", "Prix Production Sortie","Montant Production Sortie","Quantite Vente", "Prix Vente","Montant Vente","Quantite Gratuite", "Prix Gratuite","Montant Gratuite","Quantite Avoir", "Prix Avoir","Montant Avoir","Quantite Finale", "Prix Finale","Montant Finale",
       	}
       ));
       ...
       tableEtatStock.setRowHeight(20);
		...
		buttonvalider.setBounds(870, 793, 112, 32);
		...
		JLabel lblConslterLesFactures = new JLabel("           Consulter Etat Marge de Stock :");
...
if(etatstockmp.getMontantVente().compareTo(BigDecimal.ZERO)!=0 || etatstockmp.getMontantProductionSortie().compareTo(BigDecimal.ZERO)!=0)
{
	
	for(int j=0;j<listEtatMargeStock.size();j++)
	{
		
	EtatMargeStock etatMargeStock=	listEtatMargeStock.get(j);
		
	if(etatstockmp.getFamille().equals(subCategorieMP.getNom()))
	{
		
		if(etatMargeStock.getSousfamille().equals(etatstockmp.getArticle()))
		{
			existe=true;
			etatMargeStock.setMontantInitial(etatMargeStock.getMontantInitial().add(etatstockmp.getMontantInitial()));
			if(etatstockmp.getMontantProductionEntrer().compareTo(BigDecimal.ZERO)!=0)
			{
				etatMargeStock.setMontantProductionEntrer(etatMargeStock.getMontantProductionEntrer().add(etatstockmp.getMontantProductionEntrer()));
			}else if(etatstockmp.getMontantProductionSortie().compareTo(BigDecimal.ZERO)!=0)
			{
				
				etatMargeStock.setMontantProductionEntrer(etatMargeStock.getMontantProductionEntrer().add(etatstockmp.getMontantProductionSortie()));
				
			}
			
			etatMargeStock.setMontantAchat(etatMargeStock.getMontantAchat().add(etatstockmp.getMontantAchat()));
			etatMargeStock.setMontantRRR(BigDecimal.ZERO);
			etatMargeStock.setMontantRRRCredit(BigDecimal.ZERO);
			etatMargeStock.setMontantFinale(etatMargeStock.getMontantFinale().add(etatstockmp.getMontantFinale()));
			etatMargeStock.setMontantAchatRevendu(BigDecimal.ZERO);
			etatMargeStock.setMontantChiffreAffaire(etatMargeStock.getMontantChiffreAffaire().add(etatstockmp.getMontantVente()));
			etatMargeStock.setMarge(BigDecimal.ZERO);
			etatMargeStock.setPourcentage(BigDecimal.ZERO);
			
			
			
			listEtatMargeStock.set(j, etatMargeStock);
	...
	}else if(etatstockmp.getFamille().equals(LIBELLE_FAMILLE_EAU) || etatstockmp.getFamille().equals(LIBELLE_FAMILLE_EAU_GAZEUSE))
	{
		
		
	
		
		if(etatMargeStock.getSousfamille().equals(LIBELLE_FAMILLE_EAU))
		{
			
			System.out.println( etatMargeStock.getSousfamille()+ " MontantRRR :"+etatMargeStock.getMontantRRR()+   " Montant Gratuite = "+etatstockmp.getMontantGratuite() +"  : "+" Montant Avoir = "+etatstockmp.getMontantAvoir());
			...
			etatMargeStock.setMontantAchat(etatMargeStock.getMontantAchat().add(etatstockmp.getMontantAchat()));
			etatMargeStock.setMontantRRR(etatMargeStock.getMontantRRR().add(etatstockmp.getMontantAvoir().add(etatstockmp.getMontantGratuite())));
			etatMargeStock.setMontantRRRCredit(BigDecimal.ZERO);
			etatMargeStock.setMontantFinale(etatMargeStock.getMontantFinale().add(etatstockmp.getMontantFinale()));
			etatMargeStock.setMontantAchatRevendu(BigDecimal.ZERO);
			etatMargeStock.setMontantChiffreAffaire(etatMargeStock.getMontantChiffreAffaire().add(etatstockmp.getMontantVente()));
	...
	}else if(etatstockmp.getFamille().equals(LIBELLE_FAMILLE_JUS))
	{
		
		if(etatMargeStock.getSousfamille().equals(etatstockmp.getFamille()) )
		{
			existe=true;
			etatMargeStock.setMontantInitial(etatMargeStock.getMontantInitial().add(etatstockmp.getMontantInitial()));
			if(etatstockmp.getMontantProductionEntrer().compareTo(BigDecimal.ZERO)!=0)
		...
		if(etatMargeStock.getSousfamille().equals(etatstockmp.getSousfamille()))
		{
			...
			}else if(etatstockmp.getMontantProductionSortie().compareTo(BigDecimal.ZERO)!=0)
			{
				...
				etatMargeStock.setMontantProductionEntrer(etatMargeStock.getMontantProductionEntrer().subtract(etatstockmp.getMontantProductionSortie()));
	...
	//System.out.println(etatMargeStock.getSousfamille() +" : "+etatMargeStock.getMontantChiffreAffaire());	
	...
	if(existe==false)
	{
		...
		EtatMargeStock etatMargeStock=new EtatMargeStock();
		if(etatstockmp.getFamille().equals(subCategorieMP.getNom()))
		{
			etatMargeStock.setSousfamille(etatstockmp.getArticle());
			etatMargeStock.setFamille(subCategorieMP.getNom());
			etatMargeStock.setMontantRRR(BigDecimal.ZERO);
			etatMargeStock.setMontantRRRCredit(BigDecimal.ZERO);
			etatMargeStock.setMontantChiffreAffaire(etatstockmp.getMontantVente());
			
		}else if(etatstockmp.getFamille().equals(LIBELLE_FAMILLE_EAU) || etatstockmp.getFamille().equals(LIBELLE_FAMILLE_EAU_GAZEUSE))
		{
			etatMargeStock.setSousfamille(LIBELLE_FAMILLE_EAU);
			etatMargeStock.setFamille(LIBELLE_FAMILLE_EAU);
			
			etatMargeStock.setMontantRRR(etatstockmp.getMontantAvoir().add(etatstockmp.getMontantGratuite()));
			etatMargeStock.setMontantRRRCredit(etatstockmp.getMontantAvoir().add(etatstockmp.getMontantGratuite()));
			etatMargeStock.setMontantChiffreAffaire(etatstockmp.getMontantVente());
		
			System.out.println( etatMargeStock.getSousfamille()+ " Montant Gratuite = "+etatstockmp.getMontantGratuite() +"  : "+" Montant Avoir = "+etatstockmp.getMontantAvoir());

			
		}else if(etatstockmp.getFamille().equals(LIBELLE_FAMILLE_JUS))
		{
			
			etatMargeStock.setSousfamille(LIBELLE_FAMILLE_JUS);
			etatMargeStock.setFamille(LIBELLE_FAMILLE_JUS);
			etatMargeStock.setMontantRRR(BigDecimal.ZERO);
			etatMargeStock.setMontantRRRCredit(BigDecimal.ZERO);
			etatMargeStock.setMontantChiffreAffaire(etatstockmp.getMontantVente());
		}else {
			etatMargeStock.setSousfamille(etatstockmp.getSousfamille());
			etatMargeStock.setFamille(etatstockmp.getSousfamille());
			etatMargeStock.setMontantRRR(BigDecimal.ZERO);
			etatMargeStock.setMontantRRRCredit(BigDecimal.ZERO);
			etatMargeStock.setMontantChiffreAffaire(etatstockmp.getMontantVente());
		}
		
		etatMargeStock.setMontantInitial(etatstockmp.getMontantInitial());
		if(etatstockmp.getMontantProductionEntrer().compareTo(BigDecimal.ZERO)!=0)
		{
			etatMargeStock.setMontantProductionEntrer(etatstockmp.getMontantProductionEntrer());
		}else 
		
		if(etatstockmp.getMontantProductionSortie().compareTo(BigDecimal.ZERO)!=0)
		{
			
			etatMargeStock.setMontantProductionEntrer(etatstockmp.getMontantProductionSortie().multiply(new BigDecimal(-1)));
			
		}else
		{
			etatMargeStock.setMontantProductionEntrer(BigDecimal.ZERO);	
			
		}
		
		
		etatMargeStock.setMontantAchat(etatstockmp.getMontantAchat());
		
		etatMargeStock.setMontantFinale(etatstockmp.getMontantFinale());
		etatMargeStock.setMontantAchatRevendu(BigDecimal.ZERO);
		...
		listEtatMargeStock.add(etatMargeStock);
	    		...
	    		afficher_tableEtatStock(listEtatMargeStock);
	  ...
	  try {
	    ...
	    btnExporterExcel.addActionListener(new ActionListener() {
	    	public void actionPerformed(ActionEvent arg0) {
	    		...
	    		String titre="Etat de Marge Stock  "+magasin.getLibelle();
	    		String titrefeuilleexcel="Etat de Marge Stock  ";
	    ...
	    btnExporterExcel.setBounds(717, 793, 130, 32);
...
/*	void InitialiseTableauMouvementStock()
	...
	void afficher_tableEtatStock(List<EtatMargeStock> listEtatStockMP)
	{
		
		modeleEtatStock=new DefaultTableModel();
		
		modeleEtatStock.addColumn(" Stock ");
	
		for(int i=0;i<listEtatStockMP.size();i++)
		{
			modeleEtatStock.addColumn(listEtatStockMP.get(i).getSousfamille().toString());
			
			
		}

		modeleEtatStock.addColumn(" Total ");
		...
		int i=0;
		String[] ligneInitial=new String[modeleEtatStock.getColumnCount()];
		String[] ligneProduction=new String[modeleEtatStock.getColumnCount()];
		String[] ligneAchat=new String[modeleEtatStock.getColumnCount()];
		String[] ligneRRR=new String[modeleEtatStock.getColumnCount()];
		String[] ligneRRRCredit=new String[modeleEtatStock.getColumnCount()];
		String[] ligneSF=new String[modeleEtatStock.getColumnCount()];
		String[] ligneAchatRevendu=new String[modeleEtatStock.getColumnCount()];
		String[] ligneChiffreAffaire=new String[modeleEtatStock.getColumnCount()];
		String[] ligneMarge=new String[modeleEtatStock.getColumnCount()];
		String[] lignePourcentage=new String[modeleEtatStock.getColumnCount()];
		
		int j=0;
		ligneInitial[j]="SI";
		ligneProduction[j]="PROD";
		ligneAchat[j]="ACHAT";
		ligneRRR[j]="RRR";
		ligneRRRCredit[j]="RRR CREDIT";
		ligneSF[j]="SF";
		ligneAchatRevendu[j]="ACHAT REVENDU";
		ligneChiffreAffaire[j]="CA";
		ligneMarge[j]="MARGE";
		lignePourcentage[j]="%";
		...
		BigDecimal TotalStockFinal=BigDecimal.ZERO;
		BigDecimal TotalAchatRevendu=BigDecimal.ZERO;
		...
		{	
			...
			AchatRevendu=(listEtatStockMP.get(i).getMontantInitial().add(listEtatStockMP.get(i).getMontantAchat().add(listEtatStockMP.get(i).getMontantProductionEntrer()))).subtract(listEtatStockMP.get(i).getMontantRRR().add(listEtatStockMP.get(i).getMontantRRRCredit().add(listEtatStockMP.get(i).getMontantFinale())));
			marge=listEtatStockMP.get(i).getMontantChiffreAffaire().subtract(AchatRevendu);
			
			if(listEtatStockMP.get(i).getMontantChiffreAffaire().compareTo(BigDecimal.ZERO)!=0)
			{
				
				Pourcentage=marge.divide(listEtatStockMP.get(i).getMontantChiffreAffaire(), 2, RoundingMode.HALF_UP).multiply(new BigDecimal(100));
			}
			
			j=j+1;
			ligneInitial[j]=listEtatStockMP.get(i).getMontantInitial().toString();
			ligneProduction[j]=listEtatStockMP.get(i).getMontantProductionEntrer().toString();
			ligneAchat[j]=listEtatStockMP.get(i).getMontantAchat().toString();
			ligneRRR[j]=listEtatStockMP.get(i).getMontantRRR().toString();
			ligneRRRCredit[j]=listEtatStockMP.get(i).getMontantRRRCredit().toString();
			ligneSF[j]=listEtatStockMP.get(i).getMontantFinale().toString();
			ligneAchatRevendu[j]=AchatRevendu.toString();
			ligneChiffreAffaire[j]=listEtatStockMP.get(i).getMontantChiffreAffaire().toString();
			ligneMarge[j]=marge.toString();
			lignePourcentage[j]=Pourcentage.toString();
			
			
			
			TotalInitial=TotalInitial.add(listEtatStockMP.get(i).getMontantInitial());
			Totalproduction=Totalproduction.add(listEtatStockMP.get(i).getMontantProductionEntrer());
			TotalAchat=TotalAchat.add(listEtatStockMP.get(i).getMontantAchat());
			TotalRRR=TotalRRR.add(listEtatStockMP.get(i).getMontantRRR());
			TotalRRRCredit=TotalRRRCredit.add(listEtatStockMP.get(i).getMontantRRRCredit());
			TotalStockFinal=TotalStockFinal.add(listEtatStockMP.get(i).getMontantFinale());
			TotalAchatRevendu=TotalAchatRevendu.add(AchatRevendu);
			TotalChiffreAffaire=TotalChiffreAffaire.add(listEtatStockMP.get(i).getMontantChiffreAffaire());
		...
		ligneInitial[j]=TotalInitial.toString();
		ligneProduction[j]=Totalproduction.toString();
		ligneAchat[j]=TotalAchat.toString();
		ligneRRR[j]=TotalRRR.toString();
		ligneRRRCredit[j]=TotalRRRCredit.toString();
		ligneSF[j]=TotalStockFinal.toString();
		ligneAchatRevendu[j]=TotalAchatRevendu.toString();
		ligneChiffreAffaire[j]=TotalChiffreAffaire.toString();
		ligneMarge[j]=TotalMarge.toString();
		if(TotalChiffreAffaire.compareTo(BigDecimal.ZERO)!=0)
		{
			
			TotalPourcentage=TotalMarge.divide(TotalChiffreAffaire, 2, RoundingMode.HALF_UP).multiply(new BigDecimal(100));
			
		}
		lignePourcentage[j]=TotalPourcentage.toString();
		
		
		
		modeleEtatStock.addRow(ligneInitial);
		modeleEtatStock.addRow(ligneProduction);
		modeleEtatStock.addRow(ligneAchat);
		modeleEtatStock.addRow(ligneRRR);
		modeleEtatStock.addRow(ligneRRRCredit);
		modeleEtatStock.addRow(ligneSF);
		modeleEtatStock.addRow(ligneAchatRevendu);
		modeleEtatStock.addRow(ligneChiffreAffaire);
		modeleEtatStock.addRow(ligneMarge);
		modeleEtatStock.addRow(lignePourcentage);
