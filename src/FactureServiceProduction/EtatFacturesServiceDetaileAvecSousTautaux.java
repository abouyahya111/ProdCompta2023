...
import dao.entity.EtatFactureServiceProductionDetailsAvecSousTautaux;
...
import dao.entity.ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux;
...
public class EtatFacturesServiceDetaileAvecSousTautaux extends JLayeredPane implements Constantes{
	...
	private List<EtatFactureServiceProductionDetailsAvecSousTautaux> listEtatFactureServiceProductionDetailAvecSousTautaux=new ArrayList<EtatFactureServiceProductionDetailsAvecSousTautaux>();
	private List<ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux> listArticleParSousFamille=new ArrayList<ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux>();
...
private   JDateChooser audateChooser = new JDateChooser();
	 ...
	 */
	public EtatFacturesServiceDetaileAvecSousTautaux() {
		setOpaque(true);
       ...
       sousFamilleArticlesPFDAO=new SousFamilleArticlesPFDAOImpl();
		...
		buttonvalider.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String datedebut=dateFormat.format(dudateChooser.getDate());
				String datefin=dateFormat.format(audateChooser.getDate());
				
				titre="Les Factures service entre "+datedebut +" et "+datefin;
				if(listDetailFactureServiceProduction.size()!=0)
				{
						...
						JasperUtils.imprimerEtatFactureServiceAvecSousTautaux (table.getModel(), parameters); 
				...
				{
					 JOptionPane.showMessageDialog(null, "Il n'existe auccun facture service ", "Erreur", JOptionPane.ERROR_MESSAGE);
				}
		...
		buttonvalider.setBounds(666, 599, 112, 33);
		add(buttonvalider);
		  
		 
		
		JLabel lblConslterLesFactures = new JLabel("             Etat Des Factures service Avec Les Sous Tataux");
		lblConslterLesFactures.setBackground(Color.WHITE);
		lblConslterLesFactures.setFont(new Font("Verdana", Font.BOLD | Font.ITALIC, 35));
		lblConslterLesFactures.setBounds(381, 11, 1211, 35);
	    ...
	    lblDateFacture.setFont(new Font("Verdana", Font.BOLD | Font.ITALIC, 12));
	     ...
	     btnExporterExcel.addActionListener(new ActionListener() {
	     	public void actionPerformed(ActionEvent arg0) {
	    		...
	    		ExporterTableVersExcel.tabletoexcelFactureService(table, titre,titrefeuille);
...
txtNumfacture.setText("");
 InitialiseTableauFacture();	
	...
	void chargerListeFacture()
	{
		...
		listDetailFactureServiceProduction=detailfactureServiceProductiondao.findByNumFcatureClientDateFactureDepotEtatRegleOrdreByArticleBySousFamille(txtNumfacture.getText(),clientpf, dudateChooser.getDate(), audateChooser.getDate(), depot);
		
		AjouterSousFamilleAuxArticle();
		
		
		
		
		
	    afficher_tableFactureServiceProduction(listEtatFactureServiceProductionDetailAvecSousTautaux);
	...
	void afficher_tableFactureServiceProduction(List<EtatFactureServiceProductionDetailsAvecSousTautaux> listEtatFactureServiceProductionDetailAvecSousTautaux)
	{
		...
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd/MM/yyyy");
				...
				modelefacture =new DefaultTableModel(
						...
						new String[] {
								"Date facture", "Num facture", "Client", "Famille", "Sous Famille","Article","Quantite", "prix","Montant HT", "Montant TVA","Montant TTC"
						}
					) {
						boolean[] columnEditables = new boolean[] {
						...
						Class[] columnTypes = new Class[] {
		  						String.class,String.class,String.class,String.class,String.class,String.class, BigDecimal.class, BigDecimal.class, BigDecimal.class, BigDecimal.class, BigDecimal.class
		  					};
				...
				for(int d=0;d<listArticleParSousFamille.size();d++)
				{
					
					ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux=listArticleParSousFamille.get(d);
					 ...
					 MontantTotalTTC=BigDecimal.ZERO;
					  ...
					  for(int j=0;j<listEtatFactureServiceProductionDetailAvecSousTautaux.size();j++)
						{	
							
						 
							
					EtatFactureServiceProductionDetailsAvecSousTautaux detailfactureServiceProduction=listEtatFactureServiceProductionDetailAvecSousTautaux.get(j);
						
					if(ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux.getArticle().getId()==detailfactureServiceProduction.getArticle().getId())
					{
						
						if(ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux.getSousfamille().getId()==detailfactureServiceProduction.getSousfamille().getId())
						{
							  article=detailfactureServiceProduction.getArticle().getLiblle();
							  sousfamille=detailfactureServiceProduction.getSousfamille().getLiblle();
							
							QuantiteTotal=QuantiteTotal.add(detailfactureServiceProduction.getQuantite()) ;
							MontantTotal=MontantTotal.add(detailfactureServiceProduction.getMontantHT());
							MontantTotalTVA=MontantTotalTVA.add(detailfactureServiceProduction.getMontantTVA());
							MontantTotalTTC=MontantTotalTTC.add(detailfactureServiceProduction.getMontantTTC());
							if(article.contains("(OFFRE)"))
							{
								famille="OFFRE";
								Object []ligne={simpleDateFormat.format(detailfactureServiceProduction.getDateFacture())  ,detailfactureServiceProduction.getNumFacture(),detailfactureServiceProduction.getClient().getNom(),"OFFRE",detailfactureServiceProduction.getSousfamille().getLiblle(),  detailfactureServiceProduction.getArticle().getLiblle(),detailfactureServiceProduction.getQuantite(),detailfactureServiceProduction.getPrix(),detailfactureServiceProduction.getMontantHT() , detailfactureServiceProduction.getMontantTVA(), detailfactureServiceProduction.getMontantTTC() };

								modelefacture.addRow(ligne);
							}else
							{
								famille=detailfactureServiceProduction.getSousfamille().getFamileArticlePF().getLiblle();
								Object []ligne={simpleDateFormat.format(detailfactureServiceProduction.getDateFacture()) ,detailfactureServiceProduction.getNumFacture(),detailfactureServiceProduction.getClient().getNom(),detailfactureServiceProduction.getSousfamille().getFamileArticlePF().getLiblle(),detailfactureServiceProduction.getSousfamille().getLiblle(),  detailfactureServiceProduction.getArticle().getLiblle(),detailfactureServiceProduction.getQuantite(),detailfactureServiceProduction.getPrix(),detailfactureServiceProduction.getMontantHT() , detailfactureServiceProduction.getMontantTVA(), detailfactureServiceProduction.getMontantTTC() };
					  ...
					  if(MontantTotal.compareTo(BigDecimal.ZERO)!=0)
					  {
								 ...
								 Object []ligne={"SOUS TAUTAUX","","" ,famille,sousfamille,  article,QuantiteTotal,MontantTotal.divide(QuantiteTotal, 6, RoundingMode.HALF_UP),MontantTotal , MontantTotalTVA,MontantTotalTTC };
					  ...
					  }else
					  {
						  ...
						  Object []ligne={"SOUS TAUTAUX","","" ,famille,sousfamille,  article,QuantiteTotal,BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP),MontantTotal , MontantTotalTVA,MontantTotalTTC  };
	...
	public void AjouterSousFamilleAuxArticle()
	{
		listEtatFactureServiceProductionDetailAvecSousTautaux.clear();
		
		int i=0;
		
		MatierePremier THERRES=matierePremiereDAO.findByCode(Constantes.MATIERE_PREMIERE_THERRES);
		MatierePremier VERRES=matierePremiereDAO.findByCode(Constantes.MATIERE_PREMIERE_VERRES);
		...
		if(!detailfactureServiceProduction.getArticle().trim().equals(THERRES.getNom().trim()) && !detailfactureServiceProduction.getArticle().trim().equals(VERRES.getNom().trim()))
		{
			
TransferStockPF transferStockPF=transferStockPFDAO.findByCodeTransfert(detailfactureServiceProduction.getFactureService().getNumOF());	
...
Production production=productionDAO.findByNumOF(detailfactureServiceProduction.getFactureService().getNumOF(),detailfactureServiceProduction.getFactureService().getDepot().getCode());
			...
			BigDecimal montantTTC=BigDecimal.ZERO;
								...
								if(detailfactureServiceProduction.getFactureService().getMontantTVA().compareTo(BigDecimal.ZERO)!=0)
								{
									montantTVA=production.getQuantiteReel().multiply(detailfactureServiceProduction.getPrix()).multiply(new BigDecimal(0.2)).setScale(6, RoundingMode.HALF_UP);
									
									
								}
								
								montantTTC=production.getQuantiteReel().multiply(detailfactureServiceProduction.getPrix()).add(montantTVA);
								
					System.out.println(detailfactureServiceProduction.getArticle());			
			Articles articles=articleDAO.findBylibelle(detailfactureServiceProduction.getArticle());
			
			EtatFactureServiceProductionDetailsAvecSousTautaux EtatFactureServiceProductionDetailsAvecSousTautaux=new EtatFactureServiceProductionDetailsAvecSousTautaux();
			EtatFactureServiceProductionDetailsAvecSousTautaux.setDateFacture(detailfactureServiceProduction.getFactureService().getDateFacture());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setNumFacture(detailfactureServiceProduction.getFactureService().getNumFacture());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setClient(detailfactureServiceProduction.getFactureService().getClient());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setSousfamille(ListedetailTransferProduitFini.get(0).getSousFamille());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setArticle(articles);
			EtatFactureServiceProductionDetailsAvecSousTautaux.setQuantite(production.getQuantiteReel());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setPrix(detailfactureServiceProduction.getPrix());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantHT(detailfactureServiceProduction.getPrix().multiply(production.getQuantiteReel()));
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantTVA(montantTVA);
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantTTC(montantTTC);
			listEtatFactureServiceProductionDetailAvecSousTautaux.add(EtatFactureServiceProductionDetailsAvecSousTautaux);
		 	
			
			
			
			BigDecimal montantHTOffre=BigDecimal.ZERO;
			BigDecimal montantTVAOffre=BigDecimal.ZERO;
			BigDecimal montantTTCOffre=BigDecimal.ZERO;
			BigDecimal PrixOffre=BigDecimal.ZERO;
		
			
			for(int j=0;j<ListeCoutMP.size();j++)
			{
				
				CoutMP coutMP =ListeCoutMP.get(j);
				...
				.equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_THE))
			{
					...
					
				{
					  ...
					  montantTTC=BigDecimal.ZERO;
										...
										if(detailfactureServiceProduction.getFactureService().getMontantTVA().compareTo(BigDecimal.ZERO)!=0)
										{
											montantTVA=coutMP.getQuantiteOffre().multiply(detailfactureServiceProduction.getPrix()).multiply(new BigDecimal(0.2)).setScale(6, RoundingMode.HALF_UP);
											
											
										}
										
										montantTTC=coutMP.getQuantiteOffre().multiply(detailfactureServiceProduction.getPrix()).add(montantTVA);
					
					
					System.out.println(coutMP.getProdcutionCM().getNumOF());
					
					Articles article = articleDAO.findByCode(coutMP.getProdcutionCM().getArticles().getCodeArticle() + CODE_PRODUIT_FINI_OFFRE);
					
					
				 
					
					EtatFactureServiceProductionDetailsAvecSousTautaux EtatFactureServiceProductionDetailsAvecSousTautauxTmp=new EtatFactureServiceProductionDetailsAvecSousTautaux();
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setDateFacture(detailfactureServiceProduction.getFactureService().getDateFacture());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setNumFacture(detailfactureServiceProduction.getFactureService().getNumFacture());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setClient(detailfactureServiceProduction.getFactureService().getClient());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setSousfamille(ListedetailTransferProduitFini.get(0).getSousFamille());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setArticle(article);
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setQuantite(coutMP.getQuantiteOffre());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setPrix(detailfactureServiceProduction.getPrix());
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setMontantHT(detailfactureServiceProduction.getPrix().multiply(coutMP.getQuantiteOffre()));
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setMontantTVA(montantTVA);
					EtatFactureServiceProductionDetailsAvecSousTautauxTmp.setMontantTTC(montantTTC);
					listEtatFactureServiceProductionDetailAvecSousTautaux.add(EtatFactureServiceProductionDetailsAvecSousTautauxTmp);
		...
		}else if(detailfactureServiceProduction.getArticle().trim().equals(THERRES.getNom().trim()) )
		{
			
			BigDecimal montantTVA=BigDecimal.ZERO;
			BigDecimal montantTTC=BigDecimal.ZERO;
			montantTTC=detailfactureServiceProduction.getMontantHT().add(montantTVA);
			
                Articles articles=articleDAO.findBylibelle(detailfactureServiceProduction.getArticle());
			SousFamilleArticlePF sousFamilleArticlePF=sousFamilleArticlesPFDAO.findByCode(THERRES.getCode()+"_O");
			EtatFactureServiceProductionDetailsAvecSousTautaux EtatFactureServiceProductionDetailsAvecSousTautaux=new EtatFactureServiceProductionDetailsAvecSousTautaux();
			EtatFactureServiceProductionDetailsAvecSousTautaux.setDateFacture(detailfactureServiceProduction.getFactureService().getDateFacture());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setNumFacture(detailfactureServiceProduction.getFactureService().getNumFacture());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setClient(detailfactureServiceProduction.getFactureService().getClient());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setSousfamille(sousFamilleArticlePF);
			EtatFactureServiceProductionDetailsAvecSousTautaux.setArticle(articles);
			EtatFactureServiceProductionDetailsAvecSousTautaux.setQuantite(detailfactureServiceProduction.getQuantite());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setPrix(detailfactureServiceProduction.getPrix());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantHT(detailfactureServiceProduction.getMontantHT());
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantTVA(montantTVA);
			EtatFactureServiceProductionDetailsAvecSousTautaux.setMontantTTC(montantTTC);
			listEtatFactureServiceProductionDetailAvecSousTautaux.add(EtatFactureServiceProductionDetailsAvecSousTautaux);
		...
		}else if(detailfactureServiceProduction.getArticle().trim().equals(VERRES.getNom().trim()) )
		{
			...
			montantTTC=detailfactureServiceProduction.getMontantHT().add(montantTVA);
		            ...
		            Articles articles=articleDAO.findBylibelle(detailfactureServiceProduction.getArticle());
					SousFamilleArticlePF sousFamilleArticlePF=sousFamilleArticlesPFDAO.findByCode(VERRES.getCode()+"_O");
		...
		listArticleParSousFamille.clear();
	boolean existe=false;
	
		for(int t=0;t<listEtatFactureServiceProductionDetailAvecSousTautaux.size();t++)
		{
			 existe=false;
			EtatFactureServiceProductionDetailsAvecSousTautaux EtatFactureServiceProductionDetailsAvecSousTautaux=	listEtatFactureServiceProductionDetailAvecSousTautaux.get(t);
			
			for(int j=0;j<listArticleParSousFamille.size();j++)
			{
				
				if(EtatFactureServiceProductionDetailsAvecSousTautaux.getArticle().getId()==listArticleParSousFamille.get(j).getArticle().getId())
				{
					
					if(EtatFactureServiceProductionDetailsAvecSousTautaux.getSousfamille().getId()==listArticleParSousFamille.get(j).getSousfamille().getId())
					{
			...
			if(existe==false)
			{
				ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux articleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux=new ListeArticleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux();
				articleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux.setArticle(EtatFactureServiceProductionDetailsAvecSousTautaux.getArticle());
				articleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux.setSousfamille(EtatFactureServiceProductionDetailsAvecSousTautaux.getSousfamille());
				listArticleParSousFamille.add(articleParSousFamillePourEtatFactureServiceProductionAvecSousTautaux);
			}
