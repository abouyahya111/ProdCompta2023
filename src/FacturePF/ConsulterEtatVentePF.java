...
public class ConsulterEtatVentePF extends JLayeredPane implements Constantes{
	...
	private List<EtatVentePF> listEtatVentePF =new ArrayList<EtatVentePF>();
...
private   JDateChooser audateChooser = new JDateChooser();
	...
	public ConsulterEtatVentePF() {
       ...
       articleDAO=new ArticlesDAOImpl();
		...
		JLabel lblConslterLesFactures = new JLabel("                       Etat De Vente PF");
	    	...
	    	public void actionPerformed(ActionEvent arg0) {
	    		...
	    		listEtatVentePF.clear();
		    		...
		    		if(magasin!=null)
		    		{
		    			...
		    			requete=" where facturePF.magasin.id='"+magasin.getId()+"' ";
		    		...
		    		if(dudateChooser.getDate() !=null || audateChooser.getDate()!=null) {
		    			...
		    			requete=requete +" and facturePF.dateFacture between '"+dateDu +"' and  '"+dateAu+"' ";
		    			...
		    			if(clientpf!=null)
		    			{
		    				requete=requete +" and facturePF.clientPF.id = '"+clientpf.getId() +"'";
		    			}
		    			...
		    			listDetailFacturePF=detailFacturePfdao.listeDetailFacturePFByRequete(requete);
		    			...
		    			for(int i=0;i<listDetailFacturePF.size();i++)
		    			{
		    				...
		    				EtatVentePF etatVentePF=new EtatVentePF();
		    				etatVentePF.setArticle(detailFacturePF.getArticle());
		    			 	...
		    			 	etatVentePF.setFacturePF(detailFacturePF.getFacturePF());
...
dudateChooser.setCalendar(null);
 ...
 InitialiseTableauFacture();	
	...
	void afficher_tableFacturePF(List<EtatVentePF> listEtatVentePF)
	{
				...
				modelefacture =new DefaultTableModel(
						...
						new String[] {
								"Date BL", "N° BL","Date facture","Num facture", "Type", "Client","Article","Famille Article", "Sous Famille","Quantite","Prix" ,"%R","Remise Commerciale","Montant HT", "Montant TVA","Montant TTC"
						}
					) {
						boolean[] columnEditables = new boolean[] {
				...
				table.setModel(modelefacture);
				int i=0;
				BigDecimal remiseCommerciale=BigDecimal.ZERO;
				while(i<listEtatVentePF.size())
				{	
					remiseCommerciale=BigDecimal.ZERO;
				EtatVentePF detailfacturepf=listEtatVentePF.get(i);
				
				Date datefacture=detailfacturepf.getFacturePF().getDateFacture();
				if(detailfacturepf.getReduction().compareTo(BigDecimal.ZERO)!=0)
				{
					
					remiseCommerciale=((detailfacturepf.getQuantite().multiply(detailfacturepf.getPrixUnitaire())).multiply(detailfacturepf.getReduction()).divide(new BigDecimal(100), 6, RoundingMode.HALF_UP));
					
				}
				
					Object []ligne={detailfacturepf.getFacturePF().getDateBl(),detailfacturepf.getFacturePF().getNumBl(),detailfacturepf.getFacturePF().getDateFacture(),detailfacturepf.getFacturePF().getNumFacture(),detailfacturepf.getFacturePF().getType(),detailfacturepf.getFacturePF().getClientPF().getNom(),detailfacturepf.getArticle().getLiblle(),detailfacturepf.getFamilleArticle().getLiblle(),detailfacturepf.getSousFamille().getLiblle(),detailfacturepf.getQuantite(),detailfacturepf.getPrixUnitaire(),detailfacturepf.getReduction(),remiseCommerciale, detailfacturepf.getMontantHT(),detailfacturepf.getMontantTVA(),detailfacturepf.getMontantTTC()};
