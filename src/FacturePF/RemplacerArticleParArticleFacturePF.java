...
public class RemplacerArticleParArticleFacturePF extends JLayeredPane implements Constantes{
    ...
    private static XSSFCell CellOldArticle;
    private static XSSFCell CellNewArticle;
    private static XSSFCell CellSousFamille;
    private static XSSFCell CellOldSousFamille;
	 ...
	 */
	public RemplacerArticleParArticleFacturePF() {
		setOpaque(true);
...
CellOldArticle=ExcelWSheet.getRow(i).getCell(1);
CellNewArticle=ExcelWSheet.getRow(i).getCell(1);
CellPrix=ExcelWSheet.getRow(i).getCell(5);
CellQuantite=ExcelWSheet.getRow(i).getCell(4);
CellSousFamille=ExcelWSheet.getRow(i).getCell(3);

DataFormatter dataFormatter = new DataFormatter();
String formattedarticleOld = dataFormatter.formatCellValue(CellOldArticle);
String formatarticlenew = dataFormatter.formatCellValue(CellNewArticle);
String formattedNimBL = dataFormatter.formatCellValue(CellNumBL);
String formattedSousFamille = dataFormatter.formatCellValue(CellSousFamille);
String formattedOldSousFamille = dataFormatter.formatCellValue(CellOldSousFamille);


Articles articleOld=articleDAO.findByCode(formattedarticleOld);
Articles articlenew=articleDAO.findByCode(formatarticlenew);
FacturePF  facturePF=facturepfdao.findFactureVentePFByNumFacture (formattedNimBL);
SousFamilleArticlePF sousFamilleArticlePF=sousFamilleArticleDAo.findByCode(formattedSousFamille);

if(facturePF!=null && articleOld!=null && articlenew!=null)
{
	TransferStockPF transferStockPF=transferStockPFDAO.findByCodeTransfert(facturePF.getCodeTransfer());
	List<DetailFacturePF> listedetailFacturePF=detailFacturePfdao.listeDetailFacturePFByFacture(facturePF.getId());
	
	List<DetailTransferProduitFini> listedetailTransfertPF=detailTransferProduitFiniDAO.findByTransferStockPF(transferStockPF.getId());
 
	  
	if(listedetailFacturePF.size()!=0 && listedetailTransfertPF.size()!=0)
	{

	
		for(int d=0;d<listedetailFacturePF.size();d++)
		{
			DetailFacturePF detailFacturePF=listedetailFacturePF.get(d);
			
			if(detailFacturePF.getArticle().getId()==articleOld.getId())
			{
				if(detailFacturePF.getSousFamille().getId()==sousFamilleArticlePF.getId())
				{
					
				
													
													
													  detailFacturePF.setArticle(articlenew);
													  detailFacturePF.setSousFamille(sousFamilleArticlePF);
													  detailFacturePF.setQuantite(new
													  BigDecimal(CellQuantite.getNumericCellValue()));
													  detailFacturePF.setPrixUnitaire(new
													  BigDecimal(CellPrix.getNumericCellValue()));
													  detailFacturePF.setBrutHT(new
													  BigDecimal(CellQuantite.getNumericCellValue()).multiply(new
													  BigDecimal(CellPrix.getNumericCellValue())));
													  detailFacturePF.setMontantHT(new
													  BigDecimal(CellQuantite.getNumericCellValue()).multiply(new
													  BigDecimal(CellPrix.getNumericCellValue())));
													  detailFacturePF.setMontantTVA(detailFacturePF.getMontantHT().
													  multiply(new BigDecimal(0.2)));
													  detailFacturePF.setMontantTTC(detailFacturePF.getMontantHT().
		...
		for(int d=0;d<listedetailTransfertPF.size();d++)
		{
			DetailTransferProduitFini detailTransferProduitFini=listedetailTransfertPF.get(d);
			
			if(detailTransferProduitFini.getArticle().getId()==articleOld.getId())
			{
				
				detailTransferProduitFini.setArticle(articlenew);
				detailTransferProduitFini.setSousFamille(sousFamilleArticlePF);
				detailTransferProduitFini.setQuantite(new BigDecimal(CellQuantite.getNumericCellValue()));
				detailTransferProduitFini.setPrixUnitaire(new BigDecimal(CellPrix.getNumericCellValue()));
