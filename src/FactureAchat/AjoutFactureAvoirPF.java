...
public class AjoutFactureAvoirPF extends JLayeredPane implements Constantes{
	...
	TransferStockPF transferStock_R = new TransferStockPF();
	...
	private TransferStockPFDAO transferStockPFDAO_R;
...
private StockPFDAO stockpfDAO;
	...
	private FactureAchatMPDAO factureAchatMPdao;
	 ...
	 private DetailTransferProduitFiniDAO detailTransferStockPFdao_R;
	...
	public AjoutFactureAvoirPF() {
        ...
        try{ 
        	...
        	transferStockPFDAO_R=new TransferStockPFDAOImpl();
        	...
        	detailTransferStockPFdao_R=new DetailTransferProduitFiniDAOImpl();
       ...
       tableArticle.setRowHeight(20);
		...
		buttonvalider.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				FactureAvoirPF factureAvoirPF=factureAvoirpfdao.findByNumFactureByDepot(txtnumfacture.getText(),depot);
		...
		factureAvoirpf.setNumFacture(txtnumfacture.getText());
		factureAvoirpf.setFournisseur(fournisseur);
		factureAvoirpf.setDepot(depot);
		factureAvoirpf.setMagasin(magasin);
		factureAvoirpf.setDateFacture(dateChooserfacture.getDate());
		factureAvoirpf.setEtat(Constantes.ETAT_NON_REGLE);
		factureAvoirpf.setType(Constantes.TYPE_BON_LIVRAISON);
		factureAvoirpf.setMontantHT((new BigDecimal(txttotalmontantHT.getText())).setScale(6, RoundingMode.HALF_UP));
		factureAvoirpf.setMontantTVA((new BigDecimal(txttotalmontantTVA.getText())).setScale(6, RoundingMode.HALF_UP));
		factureAvoirpf.setMontantTTC((new BigDecimal(txttotalmontantTTC.getText())).setScale(6, RoundingMode.HALF_UP));	
		//factureAvoirpf.setDetailFactureAvoirPF(listDetailFactureAvoirpf);
		factureAvoirpf.setCodeTransfer(codeTransfert);
		factureAvoirpf.setCreerPar(utilisateur);
		factureAvoirpf.setDateCreer(new Date());
	    factureAvoirpfdao.add(factureAvoirpf);
	    for(int i=0;i<listDetailFactureAvoirpf.size();i++)
		 {
			 detailFactureAvoirpfdao.add(listDetailFactureAvoirpf.get(i));
		 }
	   ...
	   */
	   ...
	   /* Sequenceur  sequenceur=sequenceurDAO.findByCodeLibelle(date, Constantes.CODE_FACTURE_VENTE);
		...
		transferStock.setStatut(ETAT_TRANSFER_STOCK_AVOIR);
		transferStockPFDAO.add(transferStock);
		   ...
		   ///////////////////////////////////////////// Ajout Transfer PF Avoir Marjane /////////////////////////////////////   
		   ...
		   if(checkAvoirMarjane.isSelected()==true)
						{
							  ...
							  for(int j=0;j<listDetailTransferProduitFini.size();j++) {
								  ...
								  DetailTransferProduitFini detailTransferProduitFini_R=listDetailTransferProduitFini.get(j);
								  
								  DetailTransferProduitFini detailTransferProduitFini=new DetailTransferProduitFini();
								  detailTransferProduitFini.setArticle(detailTransferProduitFini_R.getArticle());
								  detailTransferProduitFini.setDateTransfer(detailTransferProduitFini_R.getDateTransfer());
								  detailTransferProduitFini.setMagasinDestination(detailTransferProduitFini_R.getMagasinDestination());
								  detailTransferProduitFini.setMagasinSouce(detailTransferProduitFini_R.getMagasinSouce());
								  detailTransferProduitFini.setPrixUnitaire(detailTransferProduitFini_R.getPrixUnitaire());
								  detailTransferProduitFini.setQuantite(detailTransferProduitFini_R.getQuantite());
								  detailTransferProduitFini.setSousFamille(detailTransferProduitFini_R.getSousFamille());
								  detailTransferProduitFini.setTransferStockPF(transferStock_R);
								  detailTransferProduitFini.setTypeTransfer(ETAT_TRANSFER_STOCK_AVOIR_R);
								  
							  listDetailTransferProduitFini_R.add(detailTransferProduitFini);
							  
							  }
							
							  transferStock_R.setCodeTransfer(codeTransfert+"_R");
							  transferStock_R.setCreerPar(utilisateur); transferStock_R.setDate(new
							  Date()); transferStock_R.setDateTransfer(dateChooserfacture.getDate());
							  transferStock_R.setListDetailTransferProduitFini(
							  listDetailTransferProduitFini_R);
							  transferStock_R.setStatut(ETAT_TRANSFER_STOCK_AVOIR_R);
							  transferStockPFDAO_R.add(transferStock_R);
							  
							  for(int j=0;j<listDetailTransferProduitFini_R.size();j++) {
								
								  
							  detailTransferStockPFdao_R.add(listDetailTransferProduitFini_R.get(j)); }
									        ...
									        while(i<listDetailFactureAvoirpf.size())
									        {
											  ...
											  /*        montanttotal=  montanttotal.add(detailFactureAvoirpf.getMontantTTC().divide(detailFactureAvoirpf.getArticle().getConditionnement(),RoundingMode.HALF_UP));
									        	...
									        	}else
									        	{
									        	  ...
									        	  DetailFactureAvoirPF detailFactureAvoir=listDetailFactureAvoirpf.get(i);
	...
	void afficher_tableDetailFactureAchat(List<DetailFactureAvoirPF> listDetailFacture)
	{
		...
		{	
			boolean existe=false;
		DetailFactureAvoirPF detailfactureAchat=listDetailFacture.get(i);
