...
public class ConsulterFactureAvoirPFComparaison extends JLayeredPane implements Constantes{
	...
	private List<DetailFactureAvoirPFComparaison> listDetailFactureAvoirPFTMPComparaison =new ArrayList<DetailFactureAvoirPFComparaison>();
	...
	private List<FactureAvoirPFComparaison> listFactureAvoirPFComparaison =new ArrayList<FactureAvoirPFComparaison>();
	private List<FactureAvoirPFComparaison> listFactureAvoirPFTmpComparaison =new ArrayList<FactureAvoirPFComparaison>();
	...
	private FactureAvoirPFComparaison factureAvoirPFComparaison;
...
private FournisseurDAO fournisseurdao;
	 ...
	 */
	public ConsulterFactureAvoirPFComparaison() {
		setOpaque(true);
			...
			public void actionPerformed(ActionEvent e) {
				...
				Fournisseur fournisseur=mapFournisseur.get(comboFournisseur.getSelectedItem());
				 int reponse = JOptionPane.showConfirmDialog(null, "Vous voulez vraiment Modifier La facture  ?", 
					...
					{
						FactureAvoirPFComparaison factureAvoirpfTmp=listFactureAvoirPFComparaison.get(table.getSelectedRow());
						...
						if(checkAvoirMarjane.isSelected()==true)
						{
							...
							factureAvoirpfTmp.setStatut(Constantes.STATUT_FACTURE_AVOIR_COMPARAISON_MARJAN);
							
						}else
						{
							
							factureAvoirpfTmp.setStatut(Constantes.STATUT_FACTURE_AVOIR_COMPARAISON_NORMAL);
						}
						
						
						
					    factureAvoirpfComparaisondao.edit(factureAvoirpfTmp);
    					...
    					listMagasin=depotdao.listeMagasinByTypeMagasinDepot(depot.getId(),Constantes.MAGASIN_CODE_TYPE_PF);
		     				if(listMagasin.size()!=0)
		     				{
		     					...
		     					while(i<listMagasin.size())
 		     				{
		     					...
		     					{if(listFactureAvoirPFComparaison.size()!=0)
		     					{
		     						 factureAvoirPFComparaison=listFactureAvoirPFComparaison.get(table.getSelectedRow()) ;
			     						
			     						combomagasin.setSelectedItem(factureAvoirPFComparaison.getMagasin().getLibelle());
		     					}
					...
					}else
					{
				         ...
				         if(trouve==false)
				         {
					       ...
					       //    detailFacture.setDateCreation(new Date());
					          ...
					          detailFactureAvoirPFComparaisondao.add(detailFacture);
					         ...
					         if(trouve==false)
					         {
						       ...
						       //    detailFacture.setDateCreation(new Date());
						           ...
						           */
						           ...
						           detailFactureAvoirPFComparaisondao.edit(detailFacture);
...
*/
                            ...
                            DetailFactureAvoirPFComparaison detailfactureavoirpf=listDetailFactureAvoirPFComparaison.get(tableArticle.getSelectedRow());
					  		detailFactureAvoirPFComparaisondao.delete(detailfactureavoirpf.getId());
				...
				if(table.getSelectedRow()!=-1)
				{
					...
					if(listFactureAvoirPFComparaison.size()!=0)
					{
						listStockPF.clear();
						 ...
						 if(factureAvoirPFComparaison!=null)
						 {
							 
							if(factureAvoirPFComparaison.getStatut().equals(Constantes.STATUT_FACTURE_AVOIR_COMPARAISON_MARJAN))
							{
					...
					///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	 
						...
						txtnumfacture.setText(factureAvoirPFComparaison.getNumFacture());
						dateChooserfacture.setDate(factureAvoirPFComparaison.getDateFacture());
						
						combodepot.setSelectedItem(factureAvoirPFComparaison.getDepot().getLibelle());
						
						
						combomagasin.setSelectedItem(factureAvoirPFComparaison.getMagasin().getLibelle());
						comboFournisseur.setSelectedItem(factureAvoirPFComparaison.getFournisseur().getNom());
						listDetailFactureAvoirPFComparaison=detailFactureAvoirPFComparaisondao.listeDetailFactureAvoirByFacture(factureAvoirPFComparaison.getId());
	    ...
	    btnAfficher.addActionListener(new ActionListener() {
	    	public void actionPerformed(ActionEvent arg0) {
	    		...
	    		detailFactureAvoirPFComparaisondao.ViderSession();
...
void chargerListFacture()
{
	...
	initialiser();
	InitialiseTableau();
	listFactureAvoirPFComparaison.clear();
		listFactureAvoirPFTmpComparaison=factureAvoirpfComparaisondao.findByNumFcatureFournisseurDateFactureDepotEtatRegle(txtparnumfacture.getText(),fournisseur, pardateChooser.getDate(), depot,Constantes.ETAT_NON_REGLE);
		...
		if(!comboparfamille.getSelectedItem().equals(""))
		{
			...
			if(!comboparsousfamille.getSelectedItem().equals(""))
			{
				...
				int i=0;
				while (i<listFactureAvoirPFTmpComparaison.size())
				{
					FactureAvoirPFComparaison factureavoirpf=listFactureAvoirPFTmpComparaison.get(i);
					exist=false;
					listDetailFactureAvoirPFTMPComparaison=detailFactureAvoirPFComparaisondao.listeDetailFactureAvoirByFacture(factureavoirpf.getId());
					for(int j=0;j<listDetailFactureAvoirPFTMPComparaison.size();j++)
					{
						DetailFactureAvoirPFComparaison detailfactureavoirpf=listDetailFactureAvoirPFTMPComparaison.get(j);
					...
					if(exist==true)
					{
						listFactureAvoirPFComparaison.add(factureavoirpf);
					}
				...
				{
					afficher_tableFactureAchat(listFactureAvoirPFComparaison);
				}
			}else
			{
				listFactureAvoirPFComparaison.addAll(listFactureAvoirPFTmpComparaison);
				if(listFactureAvoirPFComparaison.size()==0)
				{
					//initialiserFacture();
					InitialiseTableauFacture();
	...
	void afficher_tableDetailFactureAchat(List<DetailFactureAvoirPFComparaison> listDetailFacture)
	{
		...
		{	
			boolean existe=false;
		DetailFactureAvoirPFComparaison detailfactureAvoirPF=listDetailFacture.get(i);
	...
	void afficher_tableFactureAchat(List<FactureAvoirPFComparaison> listFacture)
	{
		modelefacture =new DefaultTableModel(
				new Object[][] {
				},
				new String[] {
						"Num Facture", "Date Facture", "Etat","Statut", "Fournisseur", "Depot", "Magasin", "Montant TTC"
				}
			) {
				boolean[] columnEditables = new boolean[] {
		...
		{	
		FactureAvoirPFComparaison factureAvoirPF=listFacture.get(i);
		
		Date datefacture=factureAvoirPF.getDateFacture();
			
			Object []ligne={factureAvoirPF.getNumFacture(),datefacture,factureAvoirPF.getEtat(),factureAvoirPF.getStatut(), factureAvoirPF.getFournisseur().getNom(),factureAvoirPF.getDepot().getLibelle(),factureAvoirPF.getMagasin().getLibelle(),factureAvoirPF.getMontantTTC()};
